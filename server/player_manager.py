...
def onCommand(sender,label,args):
    ...
    if cmd=="promote":
        ...
        if section_show == 1 or section_show == 0:
            ...
            if d["staff"] != "" and (d["locked"] < 0 or d["staff"] != d["lpgroup"]):
                sender.sendMessage("")
                required = parse(
        ...
        if section_show == 2 or section_show == 0:
            ...
            if d["punishments"] and len(d["punishments"].keys()) != 0:
                active.append("Warnings (%s rules broken, %s total warnings)" % (len(d["punishments"].keys()),sum([v["violations"] for v in d["punishments"].values()])))
...
def onTabComplete(sender,alias,args):
    ...
    if len(args)>=3:
        if any([i==cmd for i in ["suspend","staffban","demote","warn"]]):
            return [" ".join(args[3:])] + reason_tab(args[2:])
        if cmd=="punish":
